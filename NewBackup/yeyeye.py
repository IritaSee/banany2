...
logging.debug('backup target: {}'.format(asal))
...
while os.path.exists(tempat):
    logging.debug('checking existance: {}'.format(tempat))
    #os.makedirs(tempat)
...
logging.debug('{} setted as destination'.format(tempat))
...
print ("Copying...")
logging.debug('copying {} to destination, ETA {} secs'.format(uku,round(esttime,4)))
try:
...
logging.debug('process from {} to {} complete'.format(asal,tempat))
mylcd.lcd_clear()
if errvar <1:
    mylcd.lcd_display_string("BACKUP SUCCESS", 1)
    ...
    logging.debug('nyala.service should stop')
    exit()
